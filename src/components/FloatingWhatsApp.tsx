...
export default function FloatingWhatsApp() {
  ...
  const handleWhatsappClick = () => {
    const textMessage = language === 'ta'
      ...
      : "Hello Vedan Travels, I would like to inquire about booking a premium trip starting from Coimbatore.";
    
    const whatsappUrl = `https://wa.me/919363499428?text=${encodeURIComponent(textMessage)}`;
  ...
  return (
    <div id="floating-whatsapp-container" className="fixed bottom-6 right-6 z-50 flex flex-col items-end pointer-events-none">
      ...
      <motion.div
        ...
        className="mb-2 bg-luxury-black/95 text-white border border-gold/40 px-3.5 py-1.5 rounded-xl shadow-2xl backdrop-blur-md flex items-center space-x-2 pointer-events-auto cursor-pointer select-none"
      ...
      {/* Main floating action button with gold glow effect */}
      <motion.button
      ...
      >
        ...
        <span className="font-space font-semibold tracking-widest text-[#E2B13C] drop-shadow-sm flex items-center gap-1.5">
          <span>{language === 'ta' ? 'வாட்ஸ்அப் அரட்டை' : 'Chat Now'}</span>
